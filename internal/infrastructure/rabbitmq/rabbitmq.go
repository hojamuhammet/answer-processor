...
func ConsumeMessages(conn *amqp.Connection, db *sql.DB, smppClient *smpp.SMPPClient, logInstance *logger.Loggers) {
	...
	go func() {
		for d := range msgs {
			...
			service.ProcessMessage(db, smppClient, smsMessage, logInstance)
		}
	}()
